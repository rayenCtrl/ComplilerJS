...
class WhileStmt {
  ...
  execute() {
    ...
    while (!!this.evaluator.evaluate().value) {
      ...
      max--
      if (max <= 0) {
        this.errorHandler.throw(`MAXIMUM NUMBER OF LOOPS EXCEEDED`)
        break
      }
    }
  }
...
class Parser {
  ...
  handleBlock(statement) {
    let stmt = new BlockStmt(statement, new Environment(this.environment))

    return stmt
  }

  handlePrint(statement) {
    let stmt = new PrintStmt(statement, this.evaluator, this.log)

    return stmt
  }

  handleDeclaration(statement) {
    let stmt = new DeclarationStmt(statement, this.evaluator, this.environment)

    return stmt
  }

  handleIf(statement) {
    let stmt = new IfStmt(statement, this.evaluator, this.environment)

    return stmt
  }

  handleExpression(statement) {
    let stmt = new ExprStmt(statement, this.evaluator)

    return stmt
  }

  handleWhile(statement) {
    let stmt = new WhileStmt(statement, this.evaluator, this.environment)

    return stmt
  }

  handleFunction(statement) {
    let stmt = new FunctionStmt(statement, this.evaluator, this.environment)
