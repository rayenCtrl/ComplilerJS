...
class Evaluator {
  ...
  isEqualityOperator(token) {
    let operators = ['EQUALTO', 'NOTEQUALTO']
    return operators.includes(token.type)
  }
  ...
  isMultiplicationOperator(token) {
    let operators = ['MULTIPLY', 'DIVIDE', 'MODULO']
    return operators.includes(token.type)
  }

  isUnaryOperator(token) {
    let operators = ['MINUS', 'NOT', 'PLUS']
    return operators.includes(token.type)
  }
  ...
  isReserved(token) {
    let types = ['FOR', 'WHILE', 'RETURN', 'SHOW']
    return types.includes(token.type)
  }

  isForbidden(token) {
    let types = ['RBRACE', 'LBRACE', 'SEMICOLON', 'DOT', 'DECLARATOR']
    return types.includes(token.type)
  }
  ...
  handleAssignment() {
    ...
    let node = new Assignment(identifier, expression, this, this.environment)
    return node
  }
  ...
  handleBinary() {
    ...
    let node = new Binary(leftNode, this.currentToken, rightNode, this)
    return node
  }
  ...
  handleUnary() {
    ...
    let node = new Unary(this.currentToken, expr, this)
    return node
  }
  ...
  handleCall() {
    ...
    let node = new Call(callee, args, this, this.environment)
    return node
  }
  ...
  handleVariable() {
    ...
    let node = new Variable(this.currentToken, this.environment)
    return node
  }
